...
def analyze_dataframe(df, file_name, analysis_settings=None):
    ...
    with tabs[1]:  
        ...
        if missing_analysis['has_missing']:
            ...
            with col1:
                st.plotly_chart(missing_analysis['heatmap'], width='stretch')
            with col2:
                st.markdown("Missing Values Summary")
                st.dataframe(missing_analysis['summary'], width='stretch')
        else:
        ...
        st.dataframe(dtype_analysis, width='stretch')
    ...
    with tabs[3]: 
        ...
        with col1:
            ...
            st.dataframe(processed_df.head(sample_size), width='stretch')
        
        with col2:
            st.markdown(f"**Last {sample_size} rows:**")
            st.dataframe(processed_df.tail(sample_size), width='stretch')
        if analysis_settings.get('show_statistics', True):
            st.markdown("Statistical Summary")
            st.dataframe(processed_df.describe(), width='stretch')
