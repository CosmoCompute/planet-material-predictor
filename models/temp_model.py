...
def load_model():
    ...
    # Resolve output path relative to this script's directory (models/)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, "temp_model.pkl")

    # Download the model file only if it doesn't already exist
    if not os.path.exists(output_path):
        print(f"Downloading temperature model to {output_path}...")
        gdown.download(url, output_path, quiet=False)
    else:
        print(f"Temperature model already exists at {output_path}. Skipping download.")
